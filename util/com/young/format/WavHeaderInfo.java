...
import com.young.format.util.Memory;
import com.young.format.util.Sizeof;
...
public class WavHeaderInfo {
    ...
    public ArrayList<Unknown_Chunk> mUnknowsChunkList;

    private WavHeaderInfo() {
        mUnknowsChunkList = new ArrayList<Unknown_Chunk>();
    }
    ...
    public int getHeaderSize() {
        ...
        for (Unknown_Chunk uc : mUnknowsChunkList) {
    ...
    public static WavHeaderInfo parseHeader(byte[] data, int offset, int len) {
        ...
        if (BaseChunk.isChunkIDEuqla(data, offset, RIFF_Chunk.CONST_CHUNK_ID, 0)) {
            ...
            offset += instance.mRiffChunk.getChunkSize();
        }

        while (true) {
            if (BaseChunk.isChunkIDEuqla(data, offset,
                    FMT_Chunk.CONST_CHUNK_ID, 0)) {
                instance.mFmtChunk = FMT_Chunk.create(data, offset, len);
                offset += instance.mRiffChunk.getChunkSize();
            } else if (BaseChunk.isChunkIDEuqla(
                    data, offset, DATA_Chunk.CONST_CHUNK_ID, 0)) {
                instance.mDataChunk = DATA_Chunk.create(data, offset, len);
                return instance;
            } else {
                Unknown_Chunk un = Unknown_Chunk.create(data, offset, len);
                offset += un.getChunkSize();
                instance.mUnknowsChunkList.add(un);
            }
        }
    ...
    private static class BaseChunk {
        ...
        public BaseChunk doCreate(byte[] data, int offset, int len) {
            if (offset < 0 || data.length < offset + len) {
                throw new IllegalArgumentException("arrar len:" + data.length +
                        " offset:" + offset + " len:" + len);
            }
            BaseChunk chunk = new BaseChunk();
            System.arraycopy(data, 0, ChunkID, 0, ChunkID.length);
            offset += 4;
    ...
    public static class FMT_Chunk extends BaseChunk {
        ...
        @Override
        public String toString() {
            return super.toString() + " AudioFormat:" + AudioFormat +
