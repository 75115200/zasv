...
static int latm_write_header(/*AVFormatContext*/void *s)
{
    LATMContext *ctx = 0;//s->priv_data;
    //AVCodecParameters *par = 0;//s->streams[0]->codecpar;
    uint8_t *extradata = 0;
    int extradata_size = 0;

#warning "hello"
    int AV_CODEC_ID_AAC_LATM = 0;
...
static void latm_write_frame_header(/*AVFormatContext*/void *s, PutBitContext *bs)
{
    LATMContext *ctx = 0;//s->priv_data;
    ...
    if (!ctx->counter) {
        ...
        if (ctx->object_type == AOT_ALS) {
            ...
            if (!ctx->channel_conf) {
                ...
                av_assert0(ret >=
                           0); // extradata size has been checked already, so this should not fail
...
static int latm_write_packet()//AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (1)//s->streams[0]->codecpar->codec_id == AV_CODEC_ID_AAC_LATM)
#warning "todo"
        return 0;
...
//        goto too_large;
    ...
    latm_write_frame_header(0 /*s*/, &bs);
